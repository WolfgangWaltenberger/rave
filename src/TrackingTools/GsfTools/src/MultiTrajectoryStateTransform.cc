...
{
  std::cout << "[MultiTrajectoryStateTransform] AIIYYEE need an implementation at " << __LINE__ << std::endl;
  return TrajectoryStateOnSurface ();
  /*
  // const Surface& surface = geom.idToDet( DetId( tk.extra()->outerDetId()))->surface();
  // FIXME trino we just put in a cylinder at 4.3 centimeters ....
  Cylinder surface ( GlobalPoint ( 0., 0., 0. ), TkRotation<float>(), 4.3 );
...
MultiTrajectoryStateTransform::checkExtrapolator () const
{
  ...
  extrapolator_ = new TransverseImpactPointExtrapolator(field_);
  return true;
}
