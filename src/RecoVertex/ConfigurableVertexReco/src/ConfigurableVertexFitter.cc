...
CachingVertex<5> ConfigurableVertexFitter::vertex(
  const vector<reco::TransientTrack> & tracks, const reco::BeamSpot& beamSpot, 
  const reco::TransientTrack & ghost_track ) const
{
  cout << "ConfigurableVertexFitter: method vertex(tracks, ghost_track, beamSpot) is called" << endl;
  return theFitter->vertex ( tracks, beamSpot );
}
