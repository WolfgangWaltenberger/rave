...
#include "RecoVertex/VertexPrimitives/interface/TTtoVtxmap.h"
#include <map>
...
{
  theNDF = 0;
  for (typename vector<RefCountedVertexTrack>::const_iterator itk = theTracks.begin(); 
...
// Construct Track vector
  ...
  TTtoTTmap ttCovMap;
  TTtoVtxmap ttVtxMap;
  bool hasTkVtxCovs=false;
  // float theMinWeight = 0.5;
  // cout << "[CachingVertex] casting to TransientVertex: " << endl;
  ...
  for (typename vector<RefCountedVertexTrack>::const_iterator i = theTracks.begin();
    ...
    ttVect.push_back(t1);
    // cout << "[CachingVertex] vertex track has tk to vtx cov matrix: " << (**i).tkToVertexCovarianceAvailable()
    //     << endl;
    if ( (**i).tkToVertexCovarianceAvailable() )
    {
      hasTkVtxCovs=true;
      ttVtxMap[t1]=(**i).tkToVtxCovariance();
    }
    ...
    if (theCovMapAvailable) {
      for (typename vector<RefCountedVertexTrack>::const_iterator j = (i+1);
	   j != theTracks.end(); ++j) {
  ...
  if (hasTkVtxCovs) tv.tkToVtxCovariance ( ttVtxMap );
