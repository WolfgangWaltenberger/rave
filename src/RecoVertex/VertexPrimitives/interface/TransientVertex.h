...
//   Vertex::TrackPtrContainer tracks() const;
  ...
  bool tkToVtxCovarianceIsAvailable() const { return theVtxCovMapAvailable; }
  AlgebraicMatrix3M tkToVtxCovariance(const reco::TransientTrack & t ) const;
  void tkToVtxCovariance( const TTtoVtxmap cm );
...
//   void addTracks(const std::vector<reco::TransientTrack> & tracks);
  ...
  float theNDF;
  bool vertexValid;
  bool withPrior, theWeightMapIsAvailable, theCovMapAvailable, theVtxCovMapAvailable;
  bool withRefittedTracks;
  TTtoTTmap theCovMap;
  TTtoVtxmap theVtxTkCovs; 
