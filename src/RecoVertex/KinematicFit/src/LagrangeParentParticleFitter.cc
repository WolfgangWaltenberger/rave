...
LagrangeParentParticleFitter::LagrangeParentParticleFitter(const edm::ParameterSet & parameters) :
...
void LagrangeParentParticleFitter::configure(const edm::ParameterSet & parameters)
{
  // Parameters should be unnested from LagrangeParentParticleFitter:*
  theMaxDiff = parameters.getParameter<double>( "maximumValue" );
  theMaxStep = parameters.getParameter<int>( "maximumNumberOfIterations" );
}

edm::ParameterSet LagrangeParentParticleFitter::defaults()
{
...
//new expansionPoint
  ...
  {df += abs(vlp(i));}
  nstep++;
  //cout<<"df "<<df<<" maxdf "<<theMaxDiff<<endl;
 }while(df>theMaxDiff && nstep<theMaxStep);
